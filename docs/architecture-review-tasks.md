...
| ARCH-004 | P1 | TTSR | In progress | Stream policies now support bounded mock-attempt retry loops for `retry` and `retry_with_reminder`, including unreleased failed attempts, retry receipts, and generated/released/held/rewritten/blocked byte accounting. Remaining work is real provider stream holdback, cancellation/restart orchestration, latency budgets, and receipt offsets against actual streamed provider chunks. |
...
| ARCH-006 | P2 | State/history | In progress | Hot policy history now lives behind a bounded ETS-backed `Wardwright.PolicyCache` with deterministic eviction, status reporting, and LiveView/PubSub visibility for recent cache writes. Recent history threshold classification has a Gleam core. The target ETS topology is now documented as a low-write session catalog, per-session bounded history tables, and declared aggregate/index tables for cross-session facts. Remaining work is implementing that topology behind the current API, surfacing owner/index health in LiveView, durable sink persistence, replay/checkpoint strategy, and explicit behavior across restart and multi-instance deployment. |
