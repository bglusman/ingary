#!/usr/bin/env sh
# wardwright installer — generated by tinfoil 0.2.20
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/bglusman/wardwright/main/scripts/install.sh | sh
#   curl -fsSL https://raw.githubusercontent.com/bglusman/wardwright/main/scripts/install.sh | sh -s -- --version v1.2.3
#   curl -fsSL https://raw.githubusercontent.com/bglusman/wardwright/main/scripts/install.sh | sh -s -- --install-dir /usr/local/bin
set -eu
...
if fetch "$SUMS_URL" "$TMP/checksums-sha256.txt"; then
  EXPECTED="$(awk -v a="$ARCHIVE" '$2 == a || $2 == "*" a {print $1; exit}' "$TMP/checksums-sha256.txt")"
else
  die "checksums-sha256.txt is required but was not found: $SUMS_URL"
fi
